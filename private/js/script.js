...
// Zuerst holen alle Daten von Server: Kursen List, User Data
// das wird mit die Funktionen ganz unten gemacht
//
	...
	axios
		...
		.then((res) => {
			...
			if (res.data.username) {
				...
				usernameEl.innerHTML = `<div class="circleUser"></div><span> Hallo, ${res.data.username}</span>`;
				window.sharedUserData = userData;
				// console.log('was ist userDta:', userData);
				if (userData.payments.length > 0) {
					checkcourseList();
					buildCourseListForTab(courseList);
					// console.log('length payment >0');
					// document.querySelector('#booked-course1').innerText =
					// 	res.data.payments[0].courseId;
					const foundCourse = courseList.find(
						(course) => course.courseId === userData.payments[0].courseId
					);
					document.querySelector('#booked-course1').innerText =
						foundCourse.name;

					if (userData.payments[1]) {
						// document.querySelector('#booked-course2').innerText =
						// 	res.data.payments[1].courseId;
						const foundCourse2 = courseList.find(
							(course) => course.courseId === userData.payments[1].courseId
						);
						document.querySelector('#booked-course2').innerText =
							foundCourse2.name;
					}
...
/**
 * userData holen, den List mit gebuchten Kursen Bauen
 */
	...
	axios
		...
		.then((res) => {
			try {
				...
				console.log('Keine Daten für Kursen vorhanden');
			}
			document.querySelector('#booked-course1').innerText =
				'Sie haben noch kein Fitness gebucht';
...
/**
 * Bauen den liste mit gebuchten Kursen für Tab
 * @param {*} data
 */
const buildCourseListForTab = (data) => {
	console.log('was ist UserData im buildCourseList For tab', userData);
	const payments = userData.payments;
	try {
		const matchingCourses = data.filter((course1) =>
			payments.some((course2) => course1.courseId === course2.courseId)
		);
		document.querySelector('#booked-courses-list2').innerHTML = '';
...
const modalWindowReboot = () => {
	...
	// try {
	// 	document.querySelector('#bookBtn').removeAttribute('data-id');
	// 	document.querySelector('#bookBtn').removeAttribute('data-name');
	// } catch (err) {
	// 	console.log(err);
	// }
	// bookCourseButtonEl
...
deleteAccountBtn.addEventListener('click', (event) => {
	...
	document
		.querySelector('#deleteAccountBtnIrrevocably')
		.addEventListener('click', (event) => {
			axios
				.delete(HOST + '/api/users/members/myUserData')
				.then((res) => {
...
modalWindowEl.addEventListener('hidden.bs.modal', (event) => {
